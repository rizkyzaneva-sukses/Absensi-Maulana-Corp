...
import { employees, attendanceRecords, leaveRequests, notifications } from '@/lib/mock-data';
...
export default function EmployeeDashboard() {
  ...
  const myPendingRequests = leaveRequests.filter(
    l => l.employee_id === currentUser.id && l.status === 'PENDING'
  );
                ...
                myRecentAttendance.map(att => (
                  <div key={att.id} className="flex items-center justify-between py-2 border-b last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{att.check_in_time || '-'} - {att.check_out_time || '-'}</p>
                    </div>

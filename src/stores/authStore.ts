  ...
  persist(
    (set, get) => ({
      ...
      login: (email: string, password: string) => {
        ...
        if (employee.role === 'SUPER_ADMIN' || employee.role === 'DEVELOPER') {
      ...
      changeEmployeePassword: (employeeId: string, newPassword: string) => {
        ...
        if (!currentUser || (currentUser.role !== 'SUPER_ADMIN' && currentUser.role !== 'COMPANY_ADMIN' && currentUser.role !== 'COO' && currentUser.role !== 'DEVELOPER')) return false;
